...
func TestGetPokemonFromPokeApiSuccessWithMochs(t *testing.T) {
	c := require.New(t)
...
func TestGetPokemon(t *testing.T) {
	...
	expetedBodyResponse, err := ioutil.ReadFile("samples/api_response.json")
	c.NoError(err)

	var expectedPokemon models.Pokemon
	err = json.Unmarshal([]byte(expetedBodyResponse), &expectedPokemon)
	c.NoError(err)
